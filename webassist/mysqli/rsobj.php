...
// version 2.20
class WA_MySQLi_RS  {
	public function __construct($name,$conn,$maxRows=0,$hardLimit=false) {
	  if ($hardLimit) {
		$startLimit = $maxRows;
	    $maxRows = $hardLimit;
	  }
	  ...
	  $this->Results = array();
	  if (!$hardLimit) {
		 $startLimit = $this->PageNum * $this->MaxRows; 
	  }
	...
	public function debugSQL() {
	  ...
	  if ($this->MaxRows && strpos(strtoupper($debugStatement)," LIMIT ") === false) $debugStatement .= " LIMIT ".$this->StartLimit.",".$this->MaxRows;
	...
	public function execute() {
	  ...
	  if ($this->MaxRows && !preg_match("/ LIMIT \d*(\s*(\,|OFFSET)\s*\d*)?$/i",$statement,$result)) $statement .= " LIMIT ".$this->StartLimit.",".$this->MaxRows;
	  if (!$this->Prepared) {
	  ...
	  if ($this->MaxRows) {
		if (isset($_GET['totalRows_'.$this->Name])) {
		  ...
		  $this->TotalRows = intval($totalRows["TotalRows"]);
		}
  ...
  public function getDecryptedVal($col, $salt="") {
	...
	public function getPageLink($page) {
	  return $this->CurrentPage . "?pageNum_" . $this->Name . "=" . $page . $this->QueryString;
	}
