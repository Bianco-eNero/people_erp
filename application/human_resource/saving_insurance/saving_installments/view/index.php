...
if(isset($_GET['apply_percentage'])) {
					...
					$idd2=$row_Recordset1['sp_transaction_id'];

						$per=$_GET['percentage'];

mysqli_select_db($website,$database_website );
$query_Recordset344e = "UPDATE sp_installment_period SET percentage_calculated='$per'  WHERE sp_transaction_id='$idd2'";
					...
					<section class="action">
						<h1><?php echo $vSavingInstallments; ?> (<?php echo $row_Recordset2['sp_installment_period_desc']; ?>)</h1>
							...
							<fieldset>
								...
								<a class="focus" href="../close_period/?close=1&id=<?php echo $row_Recordset2['sp_installment_period_id']; ?>" style="text-decoration:none;"><input type="submit" name="create" id="create" value="<?php echo $vClosePeriod; ?>" class="focus" /></a>


								<a class="focus" href="../reset_inv/?reset_inv=1&id=<?php echo $row_Recordset2['sp_installment_period_id']; ?>" style="text-decoration:none; display: none"><input type="submit" name="create" id="create" value="<?php echo $vResetInv; ?>" class="focus" /></a>
...
?>
        <?php do {
				...
				if(isset($_GET['apply_percentage'])) {
					...
					$idd=$row_Recordset1['sp_transaction_id'];
					$percentage=$_GET['percentage'];

					$emp2=($percentage*$row_Recordset1['emp_portion_le']/100)/2;
					$co2=($percentage*$row_Recordset1['co_portion_le']/100)/2;

					if($row_Recordset1['sp_member_type_id']==2)
					{

mysqli_select_db($website,$database_website );
$query_Recordset344 = "UPDATE sp_transaction SET inv_emp_portion_le='$emp2', inv_co_portion_le='$co2'  WHERE sp_transaction_id='$idd'";
