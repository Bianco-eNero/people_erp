	...
	<table <?php echo $table_style; ?> class="o_list_view table table-sm table-hover table-striped o_list_view_ungrouped table-responsive-sm table-condensed" id="employees">
        ...
        <tbody>
				...
				?>
				  ...
				  $derw=$row_saving_installment['sp_journal_id'];
					  
					  
					  
mysqli_select_db($website,$database_website );
$query_saving_installmentw = "SELECT SUM(sp_journal_transaction.sp_db) AS DEB, SUM(sp_journal_transaction.sp_cr) AS CRE FROM sp_journal_transaction GROUP BY sp_journal_transaction.sp_journal_id";
...
$row_saving_installmentw = mysqli_fetch_assoc($saving_installmentw);
			...
			if(isset($row_saving_installment['sp_journal_id']))
			{
				  ?>
          <tr>
            <td width="100"><?php echo $row_saving_installment['sp_journal_code']; ?></td>
                    <td width="100"><?php echo $row_saving_installment['sp_journal_date']; ?></td>
                    <td><?php echo $row_saving_installment['sp_journal_desc']; ?></td>
                    <td width="90"><?php echo $row_saving_installmentw['DEB']; ?></td>
                    <td width="90"><?php echo $row_saving_installmentw['CRE']; ?></td>
                    <td>
						
                     <a style="display: none" onClick="return popup700(this, 'notes')" class="btn btn-primary btn-xs"  href="../../deposits/edit/?s=<?php echo $row_saving_installment['sp_deposit_id']; ?>"><i class="fa fa-edit"></i> <?php echo $vEdit; ?></a>
                     
                     
                      <a style="display: none"  class="btn btn-success btn-xs"  href="../../deposits/edit/?s=<?php echo $row_saving_installment['sp_deposit_id']; ?>"><i class="fa fa-eye"></i> <?php echo $vView; ?></a>                     </td>
          </tr>
                    <?php
					}
					$counter++;
