...
$query_saving_installment = "SELECT * FROM sp_journal ORDER BY sp_journal.sp_journal_code ";
...
mysql_select_db($database_localhost, $localhost);
$query_saving_installmentq = "SELECT * FROM sp_account ORDER BY sp_account_e";
...
mysql_select_db($database_localhost, $localhost);
$query_saving_installmentq2 = "SELECT * FROM sp_journal_transaction, sp_account WHERE sp_journal_transaction.sp_account_id=sp_account.sp_account_id AND sp_journal_id='$sss'";
...
$cr=0;
  ...
  $insertSQL = sprintf("INSERT INTO sp_journal_transaction (sp_account_id, sp_journal_id, sp_db, sp_cr) VALUES (%s, %s, %s, %s)",
...
mysql_select_db($database_localhost, $localhost);
$query_saving_installment = "SELECT * FROM sp_journal WHERE sp_journal_id='$ffttyy'";
$saving_installment = mysql_query($query_saving_installment, $localhost) or die(mysql_error());
$row_saving_installment = mysql_fetch_assoc($saving_installment);
$totalRows_saving_installment = mysql_num_rows($saving_installment);
...
$todaydate = date('d/m/Y', strtotime($row_saving_installment['sp_journal_date']));
...
$dodo= $todaydate;  echo $dodo; ?>">
                ...
                <div class="form-group" >
                    ...
                    <label for="inputEmail" class=" control-label "><B>#</B></label>
                      ...
                      <input disabled <?php echo $form_text; ?> class="form-control form_texty"   type="text" name="vCode" value="<?php echo $row_saving_installment['sp_journal_code']; ?>" >
                ...
                <div class="form-group" >
                    ...
                    <label for="inputEmail" class=" control-label "><B><?php echo $vDescription; ?></B></label>
                      ...
                      <input disabled <?php echo $form_text; ?> class="form-control form_texty"   type="text" name="vDesc"value="<?php echo $row_saving_installment['sp_journal_desc']; ?>" >
               ...
               <?php
				if(isset($row_saving_installmentq2['sp_journal_transaction_id']))
			{
			?>
			...
			if(isset($row_saving_installmentq2['sp_journal_transaction_id']))
			{
                  ...
                  <tr>
					...
					if($_SESSION['language']=='EN')
					{
					echo $row_saving_installmentq2['sp_account_e'];
					}
					if($_SESSION['language']=='AR')
					{
					echo $row_saving_installmentq2['sp_account_a'];
					}
					?></td>
                    <td width="100"><?php 
					if($row_saving_installmentq2['sp_db']>'0')
					{
					$db_total=$db_total+$row_saving_installmentq2['sp_db'];
					echo number_format($row_saving_installmentq2['sp_db'], 2, '.', ',');
					}
					?></td>
                    <td width="100"><?php 
					if($row_saving_installmentq2['sp_cr']>'0')
					{
					$cr_total=$cr_total+$row_saving_installmentq2['sp_cr'];
					echo number_format($row_saving_installmentq2['sp_cr'], 2, '.', ',');
					}
					 ?></td>
                    <td width="120">
                     <a onClick="return popup700(this, 'notes')" class="btn btn-primary btn-xs"  href="edit/?s=<?php echo $row_saving_installment['sp_deposit_id']; ?>"><i class="fa fa-edit"></i> <?php echo $vEdit; ?></a>
                     
                     
                      <a  class="btn btn-success btn-xs"  href="edit/?s=<?php echo $row_saving_installment['sp_deposit_id']; ?>"><i class="fa fa-eye"></i> <?php echo $vView; ?></a>                     </td>
                  </tr>
                    <?php
					}
					$counter++;
...
do {  
?>
  <option value="<?php echo $row_saving_installmentq['sp_account_id']?>"><?php
  if($_SESSION['language']=='EN')
  {
  echo $row_saving_installmentq['sp_account_e'];
  }
  
  if($_SESSION['language']=='AR')
  {
  echo $row_saving_installmentq['sp_account_a'];
  }
