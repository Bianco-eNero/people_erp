...
if(isset($_GET['apply_percentage'])) {

					$idd2=$row_Recordset1['cp_transaction_id'];

						$per=$_GET['percentage'];

mysqli_select_db($website,$database_website );
$query_Recordset344e = "UPDATE cp_installment_period SET percentage_calculated='$per'  WHERE cp_transaction_id='$idd2'";
$Recordset344e = $website->query($query_Recordset344e);
			...
			<main class="main" style="height:100%">
				<section class="control">
					<section class="action">
						<h1><?php echo $vSavingInstallments; ?> (<?php echo $row_Recordset2['cp_installment_period_desc']; ?>)</h1>

							<fieldset>
								<?php if($row_Recordset2['cp_installment_period_closed']<>'1') { ?>
								...
								<a class="focus" href="../close_period/?close=1&id=<?php echo $row_Recordset2['cp_installment_period_id']; ?>" style="text-decoration:none;"><input type="submit" name="create" id="create" value="<?php echo $vClosePeriod; ?>" class="focus" /></a>


								<a class="focus" href="../reset_inv/?reset_inv=1&id=<?php echo $row_Recordset2['cp_installment_period_id']; ?>" style="text-decoration:none; display: none"><input type="submit" name="create" id="create" value="<?php echo $vResetInv; ?>" class="focus" /></a>
...
<section class="navigation" style="<?php if($_SESSION['language']=='AR') { ?>float:left ; margin-right:400px <?php } ?>">
						...
						<fieldset class="refine" <?php if($_SESSION['language']=='AR') { ?>style="margin-left:0px <?php } ?> ">
						  ...
						  <select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu2('parent',this,0)">
	                        <option value="index.php"><i class="fas fa-filter"></i> <?php echo $vSelectPeriod; ?></option>
													...
													<?php do { ?>
													<option value="?id=<?php echo $row_Recordset3['cp_installment_period_id']; ?>"><?php echo $row_Recordset3['cp_installment_period_desc']; ?></option>
    ...
    <?php } while ($row_Recordset3 = mysqli_fetch_assoc($Recordset3)); ?>
						...
						<fieldset class="view">
							 ...
							 <span class="print">
								<a href="<?php echo $server; ?>application/human_resource/compl_pension/reports/saving_installment/?report=1&id=<?php echo $row_Recordset2['cp_installment_period_id']; ?>" target="_blank" >
...
?>
        <?php do {
				...
				if(isset($_GET['apply_percentage'])) {
					...
					$idd=$row_Recordset1['cp_transaction_id'];
					$percentage=$_GET['percentage'];

					$emp2=($percentage*$row_Recordset1['emp_portion_le']/100)/2;
					$co2=($percentage*$row_Recordset1['co_portion_le']/100)/2;

					if($row_Recordset1['cp_member_type_id']==2)
					{

mysqli_select_db($website,$database_website );
$query_Recordset344 = "UPDATE cp_transaction SET inv_emp_portion_le='$emp2', inv_co_portion_le='$co2'  WHERE cp_transaction_id='$idd'";
$Recordset344 = $website->query($query_Recordset344);
				}
...
window.location = "'.$server.'application/human_resource/compl_pension/saving_installments/"
      </script>';
