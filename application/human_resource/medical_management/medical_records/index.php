...
<section class="navigation" <?php if($this_is_pop=='1') { ?> style="display: none" <?php } ?> style="<?php if($_SESSION['language']=='AR') { ?>float:left ; margin-right:400px <?php } ?>">
...
<button id="filters"><i class="fas fa-filter"></i>Filters<i class="fas fa-caret-down"></i></button>
	...
	if(isset($row_sub_cases['case_main_id']))
	{
		do {
						?>
...
<div class="container">
    ...
    text-align: center;">
					...
					<a style="text-decoration: none; color: white" <?php if($row_sub_cases['open_sub_in_popup']=='1') { ?>href="#" onclick="popupCenter('<?php echo $server; ?>application/go/?page=<?php echo $row_sub_cases['case_code'] ; ?>&has_sub_case=1&case_filter=<?php echo $_GET['id']; ?>&ref=<?php
								...
								else
								{
								   ...
								   ?>&this_is_pop=1', 'myPop1',850,700);" <?php } else { ?> target="_blank" href="<?php echo $server; ?>application/go/?page=<?php echo $row_sub_cases['case_code'] ; ?>&has_sub_case=1&case_filter=<?php echo $_GET['id']; ?>&ref=<?php
								...
								else
								{
								   ...
								   ?>" <?php } ?>> <i class="fa fa-lg fa-arrow-circle-o-up"></i> <span class=""> <?php
						if($_SESSION['language']=='AR')
						{
		...
		<input value="<?php if($_GET['edit_now']=='1' || $_GET['duplicate_now']=='1' || $_GET['view_now']=='1') { if($_SESSION['language']=='AR') { echo $row_curr[$row_add['table_field_ar']]; } else { echo $row_curr[$row_add['table_field_en']]; } } ?>" <?php if($row_add['locked']=='1') { ?> disabled <?php }  ?> placeholder="<?php if($_GET['edit_now']=='1' || $_GET['duplicate_now']=='1' || $_GET['view_now']=='1') { if($_SESSION['language']=='AR') { echo $row_curr[$row_add['table_field_ar']]; } else { echo $row_curr[$row_add['table_field_en']]; } } ?>" <?php if($row_add['field_type']=='text') { ?>type="text"<?php } ?> <?php if($row_add['field_type']=='date') { ?>type="date"<?php } ?> id="name"  name="<?php echo $row_add['field_name']; ?>" style="width: 100%" class="text_hany"  <?php if($row_add['required']=='1') { ?> required <?php } ?>/>
		  <?php } ?>
			...
			<!-- /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
				...
				<textarea class="droid-arabic-kufi" id="summernote<?php echo $count ; ?>" name="<?php echo $row_add['field_name']; ?>" <?php if(!isset($_GET['view_now'])) { ?>cols="100%" rows="4" <?php } else { ?> cols="100%" rows="4" <?php  } ?>><?php
					...
					<script>
					    $(document).ready(function() {
					      $('#summernote<?php echo $count ; ?>').summernote({
					      placeholder: '',
					      tabsize: 2,
					      height: 300
					    });
